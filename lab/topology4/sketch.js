let sketch = function(p) {
  ...
  function init_particles() {
    for (var i = 0; i < number_of_particles; i++) {
      let r = p.cos(i / number_of_particles * p.TAU) * 500 + p.width / 2 + offset;
      let q = p.sin(i / number_of_particles * p.TAU) * 500 + p.height / 2 + offset;
      particles.push({
  ...
  function update_particles() {
    for (var i = 0; i < number_of_particles; i++) {
      ...
      prt.acc = p.createVector(0, 0).mult(0.1);
  ...
  function display_shape() {
    ...
    for (let i = 0; i < particles.length; i++) {
      //p.stroke(particles[i].col);
      p.vertex(particles[i].pos.x, particles[i].pos.y);
    }
    p.endShape(p.CLOSE);
  }
